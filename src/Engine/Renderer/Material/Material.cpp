...
{
	smart_pointers::shared_ptr<Material> Material::Create()
	{
		return Application::GetInstance()->GetRenderer()->CreateMaterial();
	}

	void Material::SetShaderProgram(const smart_pointers::shared_ptr<ShaderProgram>& shaderProgram)
	{
		if (m_ShaderProgram)
	...
	smart_pointers::shared_ptr<UniformBuffer> Material::GetUniformBuffer(std::string_view name) const
	{
	...
	smart_pointers::shared_ptr<Uniform> Material::GetUniform(std::string_view bufferName, std::string_view uniformName) const
	{
	...
	void Material::UpdateUniformBuffers(std::vector<std::pair<std::string, std::vector<std::pair<std::string, EUniformType>>>> uniformBuffers)
	{
		...
		for (auto itr = m_UniformBuffers.begin(); itr != m_UniformBuffers.end();)
		{
			...
			for (auto bufItr = uniformBuffers.begin(), end = uniformBuffers.end(); bufItr != end; bufItr++)
			{
				...
				{
					smart_pointers::shared_ptr<UniformBuffer> buf = itr->m_UniformBuffer;
		...
		{
			smart_pointers::shared_ptr<UniformBuffer> buf = UniformBuffer::Create();
